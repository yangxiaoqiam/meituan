<template>
  <dl class="s-item">
    ...
    <dd>
      ...
      <p>
        <el-rate v-model="meta.score" disabled text-color="#ff9900" score-template="{meta.score}">
        </el-rate>
        <span v-if="meta.score>4.5">很好</span>
        <span v-else-if="meta.score>4">很好</span>
        <span v-else-if="meta.score>3.5">不错</span>
        <span v-else-if="c>3">一般</span>
        <span v-else>很差</span>
        <span>{{meta.score}}分</span>
        <span class="s-item-comment-total">{{meta.commentNum}}人评论</span>
      </p>
      <p>
        <span v-for="(v,i) in meta.tab" :key="i">{{v + (i< meta.tab.length-1 ? ' |' :'' )}}</span> <span class="s-item-addr">{{meta.address}}</span>
        <span>
          <a><i class="el-icon-location"></i>查看地图</a>
        </span>
      </p>
      <p>
